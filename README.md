...
## Network Setup
Configured the server and client VM network settings to enable a second adapter for an internal network. Configured the server network configuration to set the static ip address: 192.168.10.10. The server and client machines were both configured to have the DNS point to the server ip.
